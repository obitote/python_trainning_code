...
- for循环
    ...
    - 执行流程：初始化表达式--条件表达式--根据结果(为真)执行循环体--步进表达式--条件表达式。

- 递增递减表达式
    - 递增递减表达式的优先级非常高；一般不在复杂表达式中使用。
    - `++变量名称`、`--变量名称`，计算要诀：先递增递减，再参与运算
    - 假设`a=1`，`b=++a*3`，等价于`a=a+1;b=a*3`，所以a为2，b为6。
    - `变量名称++`、`变量名称--`，计算要诀：先参与运算，再递增递减
    - 假设`a=1`，`b=a++*3`，等价于`b=a*3;a=a+1`，所以a为2，b为3。
